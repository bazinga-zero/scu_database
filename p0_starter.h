 ...
 public:
   ...
   */
     RowMatrix(int rows, int cols) : Matrix<T>(rows, cols) {
         data_ = new T * [rows];
         for (int i = 0; i < rows; i++) data_[i] = new T[cols];
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Add(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    std::unique_ptr<RowMatrix<int>> result(new RowMatrix<int> {matrixA->GetRowCount(), matrixA->GetColumnCount()});
    for (int i = 0; i < r; i++)
    {
        for (int j = 0; j < c; j++)
        {
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    std::unique_ptr<RowMatrix<T>> m(new RowMatrix<T>(r, c));
    for (int i = 0; i < r; i++) {
     ...
     for (int i = 0; i < r; i++) {
       for (int j = 0; j < c; j++) {
        ...
        for (int k = 0; k < matrixA->GetColumnCount(); k++) {
         ...
         m->SetElement(i, j, val + matrixC->GetElement(i, j)) ;
       }
     }
    return m;
  }
